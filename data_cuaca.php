        ...
        <section class="content">
					...
					<!-- general form elements -->
					  <div class="box box-primary">
						<div class="box-header">
						  <h3 class="box-title">Fetch Data Cuaca</h3>
						</div><!-- /.box-header -->
						<!-- form start -->
						<form role="form" method="POST" action="backend/parse_to_db.php" id="frmParse">
						 <div class="alert alert-info alert-dismissable">
							   <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
							   <h4><i class="icon fa fa-info"></i> Info</h4>
								  Klik Tombol Dibawah ini, untuk melakukan fetching data cuaca aktual dari 
								  BMKG.
						  </div>
						  <div class="box-footer">
							<button type="submit" class="btn btn-primary" id='btnFetch'>Fetch Data Dari Server BMKG</button>
						  </div>
						</form>
					  </div><!-- /.box -->
				</div>
			...
			<div class="row">
				<div class="col-xs-12">
					<div class="box">
						<div class="box-header">
						  <h3 class="box-title">Daftar Cuaca</h3>
						</div><!-- /.box-header -->
					</div>
					<div class="box-body">
						<table id="dataTable" class="table table-bordered table-hover">
							...
							<?php
								...
								$str = " SELECT * FROM tbl_infocuaca ORDER BY tglMulai DESC ";
								$res = $con->query($str);
								$no = 0;
								while($dt = $res->fetch_assoc()){
									...
									echo "
										<tr>
											...
											<td>{$dt['tglSampai']}</td>
											<td>{$dt['waktuMulai']}</td>
											<td>{$dt['waktuSelesai']}</td>
											<td><a href='' 
											title='Detail Data'><span class='glyphicon glyphicon-map-marker'></a></span></td>
											<td><a href='' 
    ...
    <script type="text/javascript">
	   $(document).ready(function(){
			$("#btnFetch").click(function(){
				...
				var x = confirm('Lakukan Fetching Data ?');
				if(x){
					$('#frmParse').submit();
				}else{
					// do nothing
